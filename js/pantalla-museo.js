...
function crearPantallaUbicacion(nombreLugar) {
  ...
  let posicion = {
    lat: parseFloat(lugar.geo.latitude),
    lng: parseFloat(lugar.geo.longitude),
  };
  initMap({
    position: posicion,
    zoom: 14,
  ...
  $("main").append(
    crearDiv("swiper mySwiper slider-imagen-museo")
  ...
  $("main").append(
    crearDiv("section-museo").append(
      crearArticuloMuseo(lugar, description)
        .append(
          crearDiv("botones-descripcion").append(
            crearBoton("", "escuchar-btn", "boton-verde btn-volume")
              .on("click", () => speechDescription(lugar.name, description))
              .append(
                crearImg(
                  "img/svg/volume.svg",
                  "Icono de volumen para escuchar el titulo y la descripcion",
                  "volume-icon"
                )
              )
          )
        ...
        .append(
          crearDiv("contenedor-botones-museo")
            .append(
              $("<a>")
            ...
            .append(
              $("<a>")
                ...
                .append(
                  crearImg(
                    "img/svg/boton-añadir-carrito.svg",
  ...
  if (description.split("\n").length > 1) {
    $(".botones-descripcion").append(
      crearBoton("Leer más", "leer-mas-btn", "boton boton-verde").on(
        "click",
        leerMas
      )
    );
...
function generarCarrousselFotos(fotos, type) {
  ...
  if (Array.isArray(fotos)) {
    fotos.forEach((foto) => {
      carrousselFotos.append(
        crearDiv("swiper-slide").append(
          crearImg(foto.contentUrl, foto.description, "imagen-museo")
            .attr("property", "photo")
    ...
    if (
      decodeURIComponent(window.location.search.substring(1)) == "Es Baluard"
    ) {
      carrousselFotos.append(
        crearDiv("swiper-slide").append(
          $("<video controls muted>")
            ...
            .append(
              $("<source>")
                .attr("src", "img/museos/esBaluard/esBaluard.mp4")
                .attr("type", "video/mp4")
            )
            .append(
              $("<source>")
                .attr("src", "img/museos/esBaluard/esBaluard.webm")
  ...
  if (exposiciones && exposiciones !== "") {
    ...
    exposiciones.forEach((expo) => {
      ...
      switch (typeUbi) {
        case "MovieTheater":
          expoFoto =
            "https://www.descobreixteatre.com/" + expo.image.contentUrl;
          break;
        case "Service":
      ...
      expos.append(
        $("<li>")
          ...
          .attr("typeof", "Event")
          .append(
            crearImg(expoFoto, expo.image.description).attr("property", "image")
          )
          .append(
            crearP({
              clases: "nombre",
    ...
    let exposDiv = crearSection()
      ...
      .append($("<ol>").addClass("exposiciones swiper").append(expos));
    return exposDiv;
  } else {
    return "";
  }
...
function crearFechasHorarioLugar(horario) {
  ...
  if (horarioEspañol.length > 1) {
    for (let i = 0; i < horarioEspañol.length; i++) {
      contenedor.append(
        $("<p>").html(horarioEspañol[i]).attr("content", horario[i])
      );
    }
    return contenedor;
  } else {
...
function crearTextoDirecciónLugar(lugar) {
  ...
  return crearDiv()
    ...
    .append($("<p>").html(direccion.streetAddress))
    .attr("property", "streetAddress")
    .append(
      crearDiv()
        .html(`${direccion.postalCode} ${direccion.addressLocality}`)
        .attr("property", "addressLocality")
        .append(
          $("<meta>")
            .attr("property", "postalCode")
            .attr("content", direccion.postalCode)
        )
        .append(
          $("<meta>")
            .attr("property", "addressLocality")
            .attr("content", direccion.addressLocality)
        )
    );
}
...
function crearTextoPreciosLugar(catalogoOfertas) {
  const contenedor = $("<div>").append($("<h5>").html("Precios"));
  if (!catalogoOfertas) {
  ...
  catalogoOfertas.itemListElement.forEach((offer) => {
    ...
    contenedorPrecio
      .append(
        $("<meta>").attr("property", "price").attr("content", offer.price)
      )
      .append(
        $("<div>")
          .attr("property", "itemOffered")
          .attr("typeof", "Product")
          .append(
            $("<meta>")
              .attr("property", "name")
              .attr("content", offer.itemOffered.name)
          )
      );
...
function leerMas() {
  ...
  if (parrafosAdicionales.is(":hidden")) {
    if (!primerParrafoVisible()) {
      $("#first-desc-museum")[0].scrollIntoView({
        behavior: "smooth",
        block: "start",
      }); // Desplazar hacia el inicio del primer párrafo
    }
...
function comprobarExposicionActual(expo, typeUbi) {
  const contenedor = $("<div>")
    .attr("property", "eventSchedule")
    .attr("typeof", "Schedule");
  ...
  switch (typeUbi) {
    case "MovieTheater":
      ...
      contenedor
        ...
        .append(
          $("<meta>")
            .attr("property", "startDate")
            .attr("content", expo.startDate)
        );
      break;
    case "Service":
      if (
        expo.eventSchedule.startDate <= new Date().toISOString() &&
        expo.eventSchedule.endDate >= new Date().toISOString()
      ) {
        p.addClass("fecha-verde").text(
          "Hasta el " +
            convertirFormatoFechaExposicion(expo.eventSchedule.endDate)
        );
      } else {
        p.text(
          convertirFormatoFechaExposicion(expo.eventSchedule.startDate) +
            " - " +
            convertirFormatoFechaExposicion(expo.eventSchedule.endDate)
        );
      }
      contenedor
        ...
        .append(
          $("<meta>")
            .attr("property", "startDate")
            .attr("content", expo.eventSchedule.startDate)
        )
        .append(
          $("<meta>")
            .attr("property", "endDate")
            .attr("content", expo.eventSchedule.endDate)
        );
      break;
  }
